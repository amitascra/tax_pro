...
// Extend Sales Invoice to support custom tax calculations
frappe.ui.form.on('Sales Invoice', {
	...
	refresh: function(frm) {
		...
		update_profit_margin_item(frm);
	}
});

// Store the calculated profit globally to use after item details are fetched
window.tax_pro_calculated_profit = 0;

// Feature 2: Auto-add Profit Margin Item
frappe.ui.form.on('Sales Invoice Item', {
	custom_vehicle_serial_no: function(frm, cdt, cdn) {
		console.log('=== TAX PRO: Serial number changed ===');
		update_profit_margin_item(frm);
	},
	
	custom_item_type: function(frm, cdt, cdn) {
		console.log('=== TAX PRO: Item type changed ===');
		update_profit_margin_item(frm);
	},
	...
	item_code: function(frm, cdt, cdn) {
		...
		if (row.item_code === 'Profit Margin' && window.tax_pro_calculated_profit > 0) {
			console.log('=== TAX PRO: Profit Margin item_code set, will update rate after details load ===');
			// Use setTimeout to ensure this runs after get_item_details completes
			setTimeout(function() {
				console.log('Setting Profit Margin rate to:', window.tax_pro_calculated_profit);
				frappe.model.set_value(cdt, cdn, 'rate', window.tax_pro_calculated_profit);
	...
	items_remove: function(frm, cdt, cdn) {
		console.log('=== TAX PRO: Item removed ===');
...
// Feature 2: Update Profit Margin Item
function update_profit_margin_item(frm) {
	console.log('=== TAX PRO: update_profit_margin_item called ===');
			...
			frappe.call({
				...
				callback: function(profit_result) {
					...
					if (profit_result.message) {
						...
						window.tax_pro_calculated_profit = total_profit;
